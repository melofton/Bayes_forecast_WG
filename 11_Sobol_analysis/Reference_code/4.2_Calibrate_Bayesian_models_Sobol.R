...
my_directory <- "C:/Users/Mary Lofton/Documents/RProjects/Bayes_forecast_WG/11_Sobol_analysis"
write_plots <- TRUE
...
#1) Source helper functions ---------------------------------------------------------
  ...
  source('11_Sobol_analysis/model_calibration_get_data_Sobol.R')
...
model=paste0("11_Sobol_analysis/",model_name, '.R') #Do not edit
...
write.csv(mus,file = file.path("./11_Sobol_analysis",paste0(model_name,'_predicted_states.csv')),row.names = FALSE)
...
#save results
sink(file = file.path("./11_Sobol_analysis",paste0(model_name,'_param_summary.txt')))
...
write.jagsfile(jags.out, file=file.path("./11_Sobol_analysis",paste0(model_name,'_calibration.txt')),
